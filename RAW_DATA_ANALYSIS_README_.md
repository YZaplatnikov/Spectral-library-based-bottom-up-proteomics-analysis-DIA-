...
- Downloading the DDA companion raw files from PRIDE
...
mkdir -p data_prot/PXD062422_DDA
mkdir -p fasta
...
## Download the DIA dataset

The DIA training data comes from PRIDE project **PXD062423** — Thermo `.raw` DIA files from a tear-fluid proteomics experiment plus DIA-NN output tables.
...
## Download the DDA companion dataset

The DDA raw files are stored in the companion PRIDE project **PXD062422**.

The pooled DDA files are needed for the main DIA vs DDA comparison. The DDA dilution-series files are needed for the dilution / quantification-accuracy analysis.

```bash
cd ~/projects/diann_tutorial/data_prot/PXD062422_DDA

BASE_URL="https://ftp.pride.ebi.ac.uk/pride/data/archive/2025/10/PXD062422"
```

Download the pooled DDA replicates:

```bash
for FILE in \
  Pool_DDA_R1.raw Pool_DDA_R2.raw Pool_DDA_R3.raw Pool_DDA_R4.raw \
  Pool_DDA_R5.raw Pool_DDA_R6.raw Pool_DDA_R7.raw Pool_DDA_R8.raw
do
  wget -c "${BASE_URL}/${FILE}"
done
```

Download the DDA dilution-series files:

```bash
for FILE in \
  DDA_Base_1.raw DDA_Base_2.raw DDA_Base_3.raw \
  DDA_2X_1.raw   DDA_2X_2.raw   DDA_2X_3.raw   \
  DDA_4X_1.raw   DDA_4X_2.raw   DDA_4X_3.raw   \
  DDA_8X_1.raw   DDA_8X_2.raw   DDA_8X_3.raw
do
  wget -c "${BASE_URL}/${FILE}"
done
```

Optional: download the original Proteome Discoverer `.msf` result files:

```bash
for FILE in \
  Pool_DDA_R1.msf Pool_DDA_R2.msf Pool_DDA_R3.msf Pool_DDA_R4.msf \
  Pool_DDA_R5.msf Pool_DDA_R6.msf Pool_DDA_R7.msf Pool_DDA_R8.msf \
  DDA_Base_1.msf DDA_Base_2.msf DDA_Base_3.msf \
  DDA_2X_1.msf   DDA_2X_2.msf   DDA_2X_3.msf   \
  DDA_4X_1.msf   DDA_4X_2.msf   DDA_4X_3.msf   \
  DDA_8X_1.msf   DDA_8X_2.msf   DDA_8X_3.msf
do
  wget -c "${BASE_URL}/${FILE}"
done
```
...
Convert one DDA file:

```bash
docker run --rm \
  -v ~/projects/diann_tutorial/data_prot/PXD062422_DDA:/data \
  chambm/pwiz-skyline-i-agree-to-the-vendor-licenses \
  wine msconvert /data/Pool_DDA_R1.raw --mzML --outdir /data
```
...
│   │   └── report_spike.tsv
│   └── PXD062422_DDA
│       ├── DDA_Base_1.raw  ...  DDA_8X_3.raw
│       └── Pool_DDA_R1.raw  ...  Pool_DDA_R8.raw
├── fasta
